...
# ✅ FIX #2: Use AsyncOpenAI instead of blocking OpenAI
...
# ─────────────────────────────────────────
# THREAD POOL for CPU-bound / blocking work
# ─────────────────────────────────────────
# Used to run SQLite writes and other blocking work off the async event loop
...
if SARVAM_API_KEY:
    ...
    logger.warning("⚠️  No SARVAM_API_KEY found.")


# ─────────────────────────────────────────
# TTS — Sarvam Bulbul v2
# ─────────────────────────────────────────
logger.info("✅ TTS configured for Sarvam Bulbul v2.")
...
# ✅ FIX #2: AsyncOpenAI — never blocks the event loop
...
BASE_URL          = os.getenv("BASE_URL", "https://unsuburban-unmasculinely-alesia.ngrok-free.dev").rstrip("/")
...
# ─────────────────────────────────────────
# ✅ FIX #4: Cache the system prompt — rebuild only on course changes
# ─────────────────────────────────────────
...
def get_system_prompt_with_courses() -> str:
    """Returns cached system prompt. Call invalidate_prompt_cache() after course changes."""
...
def _save_call_log_sync(call_sid: str, data: dict):
    """Synchronous DB write — always run in thread pool, never on the event loop."""
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
...
def save_call_log(call_sid: str, data: dict):
    """Non-blocking wrapper — fires DB write in thread pool."""
...
async def transcribe_audio(audio_bytes: bytes, filename: str) -> str:
    ...
    if not SARVAM_API_KEY:
        logger.warning("No SARVAM_API_KEY - STT skipped.")
        return ""
...
# ─────────────────────────────────────────
# ✅ FIX #2 + #3: Async LLM + non-blocking TTS
# ─────────────────────────────────────────
async def call_ollama_chat_async(messages, temperature=0.7, max_tokens=120) -> str:
    """Run Ollama (blocking HTTP) in thread pool."""
    def _sync():
        payload = {
            "model": OLLAMA_MODEL,
            "messages": messages,
...
async def get_ai_response(call_sid: str, user_input: str) -> Dict[str, str]:
    """
    Fully async AI response pipeline:
      1. RAG retrieval (async via thread pool)
      2. LLM call (async)
      3. Metadata save (fire-and-forget, never blocks response)
    """
    ...
    try:
        ...
        # ✅ FIX #5: RAG in thread pool (FAISS is CPU-bound)
        ...
        # ✅ FIX #2: Fully async LLM call
        if AI_PROVIDER == "ollama":
            raw_text = await call_ollama_chat_async(request_messages, temperature=0.7, max_tokens=120)
        else:
        ...
        # ✅ FIX #6: Metadata + DB save fire-and-forget — never delays the HTTP response
        ...
        save_call_log(call_sid, metadata)  # non-blocking (thread pool)
...
# ─────────────────────────────────────────
# ✅ FIX #3: TTS — run Piper in thread pool, return URL
# ─────────────────────────────────────────
async def generate_tts_audio(text: str, base_url: str, lang: str = "gu-IN") -> str:
    """
    Async TTS: Sarvam AI Bulbul v2 (async via thread pool).
    """
    os.makedirs(os.path.join("static", "audio"), exist_ok=True)
    
    if not SARVAM_API_KEY:
        logger.warning("No SARVAM_API_KEY - falling back to gTTS.")
    ...
    def _sarvam_synth():
        try:
            ...
            # Map lang to target_language_code
            lang_map = {
                "en-IN": "en-IN",
                "hi-IN": "hi-IN",
                "gu-IN": "gu-IN",
            ...
            target_lang = lang_map.get(lang, "gu-IN")
            ...
            payload = {
                ...
                "speaker": "anushka",
                "model": "bulbul:v2",
                "speech_sample_rate": 8000, # Vobiz likes 8k/16k
    ...
    filename = await asyncio.get_event_loop().run_in_executor(_executor, _sarvam_synth)
    if filename:
...
async def _gtts_fallback(text: str, lang: str, base_url: str) -> str:
    ...
    filename = await asyncio.get_event_loop().run_in_executor(_executor, _gtts_synth)
    return f"{base_url}/static/audio/{filename}" if filename else ""


# ─────────────────────────────────────────
# XML BUILDER HELPERS  (now async)
# ─────────────────────────────────────────
async def gather_xml(request: Request, speak_text: str, action_path: str, lang: str = "gu-IN") -> str:
...
# ─────────────────────────────────────────
# ✅ FIX #1: Download recording async + parallel STT + LLM
# ─────────────────────────────────────────
async def download_recording_async(recording_url: str) -> bytes:
    """Downloads recording in thread pool with retries for empty files."""
    def _dl():
        ...
        for attempt in range(3):
            ...
            if resp.status_code == 200 and len(resp.content) >= 500:
                return resp.content
            if attempt < 2:
                time.sleep(0.5) # Wait for provider to finish writing
        # Final attempt
        resp = http_session.get(recording_url, headers=headers, timeout=HTTP_TIMEOUT_SECONDS)
        return resp.content if resp.status_code == 200 else b""
    return await asyncio.get_event_loop().run_in_executor(_executor, _dl)
...
async def vobiz_respond(request: Request):
    """
    Hot path — every millisecond counts here.
    Pipeline:
      1. Download recording (async)
      2. STT via Groq Whisper (async, ~300ms)   ← was Sarvam ~1500ms
      3. LLM via AsyncOpenAI (async, ~400-800ms) ← was blocking sync
      4. TTS via Piper thread pool (async, ~100ms)
      5. DB save fire-and-forget                 ← was blocking sync
    """
    ...
    recording_url = (
        form_data.get("RecordUrl")
        or form_data.get("RecordFile")
    ...
    if not user_speech:
        xml = await gather_xml(request, "માફ કરશો, મને બરાબર સમજાયું નથી. શું તમે ફરીથી કહી શકશો?", "vobiz-respond", lang="gu-IN")
        return Response(content=xml, media_type="text/xml", headers=get_ngrok_headers())

    # ── LLM + TTS (parallel where possible) ──────────────────────────────────
    # ✅ LLM and RAG run concurrently with each other (already inside get_ai_response)
...
async def vobiz_silent(request: Request):
    ...
    if count >= 2:
        ...
        xml = await hangup_xml(request, "એવું લાગે છે કે તમે અત્યારે ઉપલબ્ધ નથી. અમે તમને પછીથી કોલ કરીશું. આવજો!", lang="gu-IN")
    else:
        xml = await gather_xml(request, "શું તમે હજી ત્યાં છો? કૃપા કરીને કંઈક બોલો અને મને તમારી મદદ કરવામાં આનંદ થશે.", "vobiz-respond", lang="gu-IN")
    ...
    for index, phone in enumerate(phone_numbers):
        with campaign_lock:
            ...
            if not campaign or campaign.get("stop_requested"):
                if campaign:
                    campaign.update({"status": "stopped", "current_index": None,
                                     "current_phone": None, "current_call_sid": None,
                                     "current_call_status": None, "ended_at": datetime.now().isoformat()})
                return
        ...
        try:
            ...
            while True:
                ...
                if time.time() - started > CALL_MAX_DURATION_SECONDS:
                    ...
                    with campaign_lock:
                        campaign = campaigns.get(campaign_id)
                        if campaign:
                            campaign["results"][-1].update({"status": "failed", "end_reason": "campaign_timeout"})
                    break
            ...
            with campaign_lock:
                campaign = campaigns.get(campaign_id)
                if campaign:
                    campaign["results"].append({"phone_number": phone, "status": "failed", "error": str(e)})
        finally:
            with campaign_lock:
                campaign = campaigns.get(campaign_id)
                if campaign:
...
async def add_course(course: Course):
    ...
    cid = c.lastrowid
    conn.close()
    invalidate_prompt_cache()  # ✅ Keep cache fresh after course changes
...
async def update_course(course_id: int, course: Course):
    ...
    conn.commit()
    conn.close()
    invalidate_prompt_cache()  # ✅ Keep cache fresh
    return {"success": True}
...
if __name__ == "__main__":
    ...
    logger.info(f"🚀 GuniVox Backend on port {port} | answer_url={BASE_URL}/vobiz-answer")
